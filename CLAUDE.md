...
- **Bagel Factory** (UK): offizielle „Full Ingredient List"-PDF (Issue 20, 13/04/2026). KEIN Crawler. `py -3 bagelfactory-extract.py` (pdfplumber; Footer-Strip an „* Received frozen", Einträge über Seitengrenzen ok) → `data/bagelfactory-raw.json`; `node bagelfactory-update.js` → BAGELFACTORY-Block. À la carte (AC-Familie)
  ...
  - **Bun-Wahl (04.07.2026)**: die 6 Deliveroo-Buns (Plain/Poppy/Sesame/Multigrain/Everything/Cheese and Jalapeno) haben eigene Per-Portion-Werte (`BAGELFACTORY.buns[]`, `plain:true`-Referenz). Für die **vollwertigen Bagels** (`bunSwap:true` = cats spread/breakfast/veggie/seafood/deli) rechnet der Optimizer je erlaubtem Bun eine **Variante** = **Bagel − Plain Bun + gewählter Bun** (`bfSwap`; math. exakt validiert am Cream Cheese Bagel = Plain Bun 115g/296 + Cream Cheese 50g/139). Mini-Bagels (Mini-Bun ohne Alternativen) + Sweets: keine Bun-Wahl. `optimizeBagelFactory(…,bunsOk)` expandiert den Pool zu Bun-Varianten (Name-Suffix „(Multigrain)" etc.; Plain bleibt suffixlos); `bunsOk` = Subway-artige Auswahl (null/leer = alle Buns). UI: Bun-Chips (Mehrfachauswahl, mind. 1 aktiv) + „all buns"-Reset, Default **nur Plain vorgewählt** (wie Subways Single-Brot-Default; Menü-Bagels erscheinen wie beworben auf Plain — weitere Buns anwählen → Optimizer swappt je Bagel zum best-passenden; Review-Fix 04.07.2026, statt „alle an" um ungefragte Bun-Swaps zu vermeiden). In All/Accurate: `bunsOk=null` (alle Buns, damit der Cross-Restaurant-Mix den besten Bagel findet)
