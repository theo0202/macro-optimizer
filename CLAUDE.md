...
- **Pho** (UK, vietnamesisch): offizielle „Pho Nutritional Guidelines 2026" PDF (Stand 31/01/2026, 9 Seiten). KEIN Crawler, KEIN Parser — die PDF-Tabelle ist zu unregelmäßig (mehrzeilige Items, Sektions-Header, Dual-Werte) → **hand-transkribiert** in `data/pho-raw.json` (Quelle der Wahrheit, wie GDK/TFC). `node pho-update.js` → PHO-Block (Marker `__PHO_DATA_START__`/`__PHO_DATA_END__`). À la carte (AC-Alias)
  ...
  - **100 Items, 9 Kategorien**: Starters & Sides (16), Pho noodle soup (27), Curry (12), Rice bowls (6), Wok fried rice & noodles (8), Vermicelli/bún (13), Salads (11), Rice (2: broken/cauliflower — Curry/Rice-Bowls sind „excludes rice", Reis als eigene Items), Sauces (5, `sauce:true`, **Kategorie default AUS**)
...
- **Pho**: `PHO.cats[]` + `PHO.items[]` (AC-Schema; 100 Items, volle Makros AUSSER `salt:0`; `sauce:true` auf den 5 Dip-Saucen). `optimizePho(t,mode,p,activeCats,maxN)` — nur Kategorie-Chips + Max-Items (keine Schalter; Sauces-Kategorie default AUS). Generiert via `node pho-update.js` aus `data/pho-raw.json`
...
- 9 Kategorien (100 Items): Starters & Sides (16), Pho noodle soup (27), Curry (12), Rice bowls (6), Wok fried rice & noodles (8), Vermicelli/bún (13), Salads (11), Rice (2), Sauces (5)
- **Sauces-Kategorie default AUS** (Dip-Saucen, `sauce:true`); die übrigen 8 default AN
- **Curry + Rice bowls sind „excludes rice"** → Reis (broken/cauliflower) ist eine eigene „Rice"-Kategorie; der Optimizer kombiniert z.B. „Curry - Chicken" + „Broken rice" zur vollen Mahlzeit
...
- **Pho**: alle 8 Food-Kategorien aktiv (Starters/Pho/Curry/Rice bowls/Wok/Vermicelli/Salads/Rice) — **Sauces-Kategorie default AUS**, max. 5 Items; à la carte (keine Schalter); volle Makros AUSSER Salt (salt=0). Schalentier permanent gefiltert (Allergie); Curry/Rice-Bowls „excludes rice" → Reis als eigene Items
...
└── data/
    ...
    ├── pho-raw.json          ← Pho-Daten (hand-transkribiert aus der offiziellen Pho Nutritional Guidelines 2026 PDF): 100 Items, 9 Kategorien, 7 Makros (salt=0); _meta.anomalies (PDF-Inkonsistenzen wörtlich)
