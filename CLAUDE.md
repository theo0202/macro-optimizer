...
- **Bagel Factory**: `BAGELFACTORY.cats[]` + `BAGELFACTORY.buns[]` (6 Buns, `plain:true`-Referenz) + `BAGELFACTORY.items[]` (AC-Schema; 43 Items, `bunSwap:true` auf vollwertigen Bagels, volle 8 Makros per Portion, Werte = Plain Bun). `optimizeBagelFactory(t,mode,p,activeCats,maxN,noSweets,bunsSpec,noPork)` — Kategorie-Chips + Max-Bagels (1/2/3/4/5/∞) + „No snacks & sweet treats" (`noSweets`) + **Bun je Bagel-Slot** (`bunsSpec`: `null`=alle Buns · Objekt=globaler Filter/∞ · Array=per-Slot; `bfSlotPool`/`bfSlotOptimize` für gemischte Slots, sonst Global-Pfad via `bfSwap` = Bagel − Plain Bun + Bun) + Schalter „No smoky pulled pork" (`noPork` filtert `id==="smoky_pulled_pork"`, **Default AUS**). State `bfBunSlots` (Array, Default je Slot nur Plain) + `setBFSlot(i,val)`; UI = maxBF Slot-Wähler (∞ → 1 globaler), Subway-Style „all buns"-Chip
...
Alle Filter-/Exclude-Checkboxen sind beim App-Start **aktiviert**, damit der User sie nicht jedes Mal neu anschalten muss: Subway "Keine Sauce"+"No Roast Chicken Breast" ("No cheese" startet AUS — User 20.06.2026, Käse standardmäßig erlaubt), Farmer J "Nur Gratis-Items", Itsu "No soups, desserts, snacks etc.", Pret "only relevant items, no bullshit", Nando's "No desserts/Lunch Fix/platters"+"No sauces"+"No grilled pineapple"+"No wings / chicken livers"+"No Corn on the Cob", Wagamama "No Ramen", GDK "No Sauce"+"No rice bowl", Urban Greens 'No "2 Toppings" / Nuts etc.'+"No Dressing"+"Max 1× Tajin/Pickled Onions/Pickled Cabbage", Atis "No sauce"+"No crunch", The Fitness Chef "No fish", Pepe's "No sauce"+"No flavour", Five Guys "No sauce", Wasabi "No sushi or soups & w/o sauces (good meals only)" (der einzige aktive Wasabi-Schalter; "No soups" startet hier AUS, weil "good meals only" Soup ohnehin ausschließt), Bagel Factory "No snacks & sweet treats". Pizza Express hat keine Schalter, aber die Desserts-Kategorie startet AUS.
