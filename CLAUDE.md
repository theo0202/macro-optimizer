...
- **The Sushi Co**: `SUSHICO.cats[]` + `SUSHICO.items[]` (AC-Schema; 78 Items, 13 Kategorien, volle 8 Makros, PORTIONS-Werte). `optimizeSushiCo(t,mode,p,activeCats,maxN)` — nur Kategorie-Chips + Max-Items (keine Schalter). Generiert via `node sushico-update.js` aus `data/sushico-raw.json`. Schalentier via Name + „rainbow roll" in SHELLFISH_NAMES
...
└── data/
    ...
    ├── sushico-raw.json      ← The-Sushi-Co-Daten (bild-transkribiert aus 2 Naehrwert-Bildern): 78 Items, 13 Kategorien, volle 8 Makros (PORTIONS); _meta.note (Atwater-inkonsistent bei frittierten) + _meta.shellfish/caveat
