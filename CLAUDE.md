...
4. Restaurant-Tabs (**➕ Add own order** / **🛒 Waitrose** / **Accurate restaurants** / **All restaurants** / Subway / Farmer J / Itsu / Pret / Nando's / Urban Greens / Wagamama / GDK / Atis / Fitness Chef / Chopstix / Pepe's / Five Guys / Pizza Express / Wasabi / Leon / Bagel Factory / Pho / Wingstop / The Sushi Co)
...
- **`SEARCH_INDEX`** (Modul-Level, `buildSearchIndex()`): flache, durchsuchbare Liste ALLER Food-Items/Komponenten mit echten 8 Makros aus allen 20 Restaurants (~1330 Einträge nach Dedup; Pho salt=0). Sammelt `.items` der AC-Restaurants + alle Komponenten-Arrays von Subway/FJ/UG/Atis/Five Guys + Chopstix-Toppings + Chopstix-Bases (zu „… (Regular)"/„… (Large)" expandiert). **Bewusst ausgelassen**: Pepe's `flavours` (per-10 ml), Five-Guys-`mods` (Bun/Patty-Deltas, teils negativ), Atis `basesL`/`saucesL` (ungenutzte Large-Dubletten). Dedup per `resto|name|kcal`. Jeder Eintrag: `{resto (Anzeigename), name, kcal, fat, sat, carbs, sugars, fibre, protein, salt}`
...
- **`WAITROSE.items[]`** (Modul-Level): je `{ id, name, brand, variable, g (typisches/fixes Gewicht), p100:{8 Makros per 100g} }`. Start: 3 Sushi-Daily-Produkte (Deluxe Duo Poke 360g, Edamame 93g, Yakisoba Chicken 326g) + 1 Waitrose-Produkt (Sweet Chilli Chicken Mini Fillets 165g fix). Weitere Produkte einfach als Objekte ergänzen
