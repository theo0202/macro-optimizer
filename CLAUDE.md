...
## „🛒 Waitrose" — Supermarkt-Tracker mit editierbarem Gewicht (`resto==="waitrose"`)
Eigener Tab **„🛒 Waitrose"** (in der Tab-Zeile direkt nach „➕ Add own order") zum **Tracken gekaufter Supermarkt-Produkte** — wie „Add own order", aber die Produkt-Nährwerte sind **per 100 g** hinterlegt und werden mit einer **editierbaren Gramm-Zahl** skaliert. Kein Optimizer.
...
- **UI** (gerendert wenn `resto==="waitrose"`): Suchfeld → Trefferliste (grünes Brand-Badge #86efac, per-100g-Makros + „typ./fixed"-Gewicht + „+ Add") → „My basket" (je Eintrag: Brand-Badge + Name + **skalierte** Makros × qty + Zeile „Weight/Fixed [Zahlfeld] g" + −/qty/+ + ✕) → „Basket total vs target"-Karte mit `MacroBar`s gegen die aktuellen Ziele. Header-Gradient Grün (#3f7d3a→#14401c), Label „WAITROSE · SUPERMARKET"
- **Guards wie „Add own order"**: `AC` ist auch für `resto==="waitrose"` `null` → Optimizer-Ergebnisliste/-Header + Corn-Cakes-Block sind mit `(resto!=="search"&&resto!=="waitrose")` geguardet. Tests in `tests.js` (16 Checks): Katalog-Counts, wtScale-Skalierung (inkl. ungültige Gramm → typisch), fix vs. variabel, waitroseSearch, waitroseTotal
