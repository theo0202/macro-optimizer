...
- **The Sushi Co** (UK): Nährwerttabelle als **2 Bilder** (User 11.07.2026) — KEIN Crawler, **bild-transkribiert** in `data/sushico-raw.json` (Quelle der Wahrheit). `node sushico-update.js` → SUSHICO-Block (Marker `__SUSHICO_DATA_START__`/`__SUSHICO_DATA_END__`). À la carte (AC-Alias). Volle 8 Makros (**PORTIONS-Spalte**; die Tabelle hat auch 100g-Spalten)
  - **82 Items, 13 Kategorien**: Hosomaki, Crunch, Futomaki, Uramaki, Sashimi, Tataki, Nigiri, Aburi & Dragon Rolls, Hot Meals (16), Poke Bowls (4), Bao, Salads, Desserts
...
- **The Sushi Co**: `SUSHICO.cats[]` + `SUSHICO.items[]` (AC-Schema; 82 Items, 13 Kategorien, volle 8 Makros, PORTIONS-Werte). `optimizeSushiCo(t,mode,p,activeCats,maxN)` — nur Kategorie-Chips + Max-Items (keine Schalter). Generiert via `node sushico-update.js` aus `data/sushico-raw.json`. Schalentier via Name + „rainbow roll" in SHELLFISH_NAMES
...
└── data/
    ...
    ├── sushico-raw.json      ← The-Sushi-Co-Daten (bild-transkribiert aus 2 Naehrwert-Bildern): 82 Items, 13 Kategorien, volle 8 Makros (PORTIONS); _meta.note (Atwater-inkonsistent bei frittierten) + _meta.shellfish/caveat
