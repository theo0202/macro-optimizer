...
- **The Sushi Co**: `SUSHICO.cats[]` + `SUSHICO.items[]` (AC-Schema; 71 Items, 12 Kategorien, volle 8 Makros, PORTIONS-Werte). `optimizeSushiCo(t,mode,p,activeCats,maxN)` — nur Kategorie-Chips + Max-Items (keine Schalter). Generiert via `node sushico-update.js` aus `data/sushico-raw.json`. Schalentier via Name + „rainbow roll" in SHELLFISH_NAMES
...
- **The Sushi Co**: alle 13 Kategorien aktiv, max. 5 Items; à la carte (keine Schalter); volle 8 Makros (PORTIONS-Werte); Schalentier permanent gefiltert (prawn/ebi/crab/octopus/california + Rainbow roll)
