...
**Ausnahme**: Die **restaurantsübergreifenden** Corn-Cake-Schalter „**Top up carbs with corn cakes**" (`cornCakes`, manuell) UND „**Smart: only when needed**" (`cornAuto`) (unter Fibre/Salt, gegenseitig exklusiv) starten beide **AUS** (User 30.06.2026 — opt-in). Der verschachtelte Cap „Max % of carbs from corn cakes" (`cornCap`) startet auf **No limit** (0). Siehe Abschnitt „Corn Cakes".
...
## Corn Cakes — restaurantsübergreifender Carb-Füller (2 Schalter: „Top up carbs…" + „Smart: only when needed")
„**Kallo Lightly salted low fat corn cakes**" (Reiswaffel-artig, fast reine Carb-Quelle) können als ganzzahliger Zusatz zu JEDER Restaurant-Bestellung berücksichtigt werden, um offene Carbs zu füllen. Globaler Block direkt unter Fibre/Salt (immer sichtbar, restaurant-unabhängig). **Zwei Modi (gegenseitig exklusiv, beide Default AUS):**
  - **„Top up carbs with corn cakes"** (`cornCakes`, manuell): hängt immer die score-minimierende Stückzahl an, wenn sie hilft.
  - **„Smart: only when needed"** (`cornAuto`): hängt Cakes NUR an, wenn das beste No-Cake-Ergebnis des Restaurants **schlecht** ist (Score > `CORN_GOOD=0.5`) — und dann **so wenige wie möglich**, um die Makros zu treffen (`smartCornCakes`: kleinste Stückzahl mit Score ≤ CORN_GOOD; „gut" unerreichbar → bestes Bemühen, 0 wenn Cakes nichts bringen). Gibt es schon ein gutes Ergebnis, bleibt die Liste cake-frei. Eine Status-Zeile zeigt „● Active …" / „○ Not needed …". (User 30.06.2026 — „nur wenn sonst nur schlechte Scores rauskommen".)
  - Das Gate sitzt in `applyCornCakes(…,auto)`: `auto && min(score der No-Cake-Liste) ≤ CORN_GOOD` → Liste unverändert. `cornAutoActive` (Component-Scope, aus `activeRaw`) steuert dasselbe live fürs Detail-Panel. **`CORN_GOOD` ist die einzige empirisch kalibrierte Schwelle** (verifiziert: P45/C95/F18 trifft Subway → 0 Cakes/„Not needed"; P40/C120/F16 → 6 Cakes; manuell würde dasselbe Ziel cake-n).
...
- **`withCornCake(r,on,t,mode,p,capPct,auto)`** / **`applyCornCakes(arr,on,t,mode,p,capPct,auto)`**: augmentiert ein Ergebnis um die Stückzahl (`auto`→`smartCornCakes`, sonst `bestCornCakes`) — neue `nutrition` (inkl. Cakes, für Karten + Ranking), `.corn` (Stückzahl), `._base` (corn-freie nutrition für Live-Recompute) + neuer `score`. `applyCornCakes(on)` **re-sortiert nach dem corn-inklusiven `score`** (sonst zeigt die Default-„Score"-Sortierung die PRE-corn-Optimizer-Reihenfolge; die Top-20-Begrenzung der Optimizer bleibt). Bei `auto` zusätzlich das Smart-Gate (s.o.). `on=false` → Array/Objekt **unverändert** (Identität bleibt)
- **EIN Choke-Point**: `activeResults`-Memo wrappt die aktive Ergebnisliste (`activeRaw`) einmal mit `applyCornCakes(activeRaw, cornCakes||cornAuto, …, cornCap, cornAuto)` → ALLE Karten (alle Restaurants inkl. All/Accurate) zeigen automatisch die kombinierten Makros + ein **Karten-Badge** „+N× corn cakes" (`cornLbl(r)`); Ranking/Sortierung berücksichtigt die Cakes. Memoisiert → stabile Objekt-Identität (Selektion-Markierung `sel===r` bleibt heil). Der Block ist im „Add own order"-Tracker (`resto==="search"`) **ausgeblendet** (`resto!=="search"&&`, dort kein Optimizer → wirkungslos)
- **Detail-Panels rechnen LIVE**: Helper **`cornTot(base)`** (Component-Scope) berechnet die Stückzahl live aus `cornCakes`/`cornAutoActive` + aktuellem `targets` + `cornCap` (im Smart-Modus `smartCornCakes`) → reagiert sofort auf Ziel-/Cap-/Modus-Änderung/Toggle (kein Stale-Panel). Pro Panel ein `cc*`-Konstrukt (`ccSub`/`ccFJ`/`ccAC`/`ccUG`/`ccAt`/`ccCh`/`ccFG`); `base` = corn-freie nutrition (`selTotal`/`selFTotal` sind corn-frei; sonst `sel._base||sel.nutrition`). Alle MacroBars nutzen `cc*.total`, die Order-Guides bekommen via **`cornStep(cc)`** einen Schritt „Supplement: N× …", die Komponenten-Breakdowns eine Corn-Zeile
- **Gilt für ALLE 16 Restaurants + All/Accurate** (im Cross-Restaurant-Mix werden die Cakes nach dem per-Restaurant-Best-Pick auf die Anzeige augmentiert; das **Smart-Gate arbeitet PRO Restaurant** — `cornGroupBest(arr)` gruppiert nach `_resto`, nur Gruppen mit bestem No-Cake-Score > CORN_GOOD bekommen Cakes; ein gutes Restaurant lässt die anderen unberührt; `selectAcross` reicht das augmentierte Objekt inkl. `._base`/`.corn` an das jeweilige Panel weiter). Der „Add own order"-Tracker (`resto==="search"`) ist NICHT betroffen (kein Optimizer → `activeRaw=[]`)
- **Bekannte Grenze (Review, akzeptiert für v1)**: Ist ein Ergebnis nur wegen Protein/Fett-Überschuss schlecht (Cakes können das nicht fixen) und Carbs sind ebenfalls offen, füllt der Smart-„best effort" die Carbs bis ans Ziel (kann viele Cakes ergeben); der %-Cap begrenzt das, und die MacroBars zeigen das Fett-/Protein-Problem ehrlich
- Tests in `tests.js` (32 Checks): Kallo-Name, per-Stück-Makros, offene Carbs → >0, über-Ziel/ohne-Ziel/Kalorien-Modus → 0, Augmentierung speichert `corn`+`_base`, `applyCornCakes(off)` = Identität, `applyCornCakes(on)` re-sortiert nach corn-Score, **Cap**: `cornCapCount`-Mathe (floor)/strikt ≤ capPct%/ungecappt > gecappt/bindet-nicht-wenn-Optimum-darunter, **Smart**: fewest-to-good ≤ Score-Min/erreicht ≤CORN_GOOD/eine-weniger-nicht-gut/gute-Basis→0/Gate (gut vorhanden→unverändert, alle schlecht→Cakes)/manuell ignoriert Gate/**per-Restaurant-Gate** (gutes Restaurant cake-frei, schlechtes bekommt Cakes)
