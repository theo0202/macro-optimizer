...
- **Bagel Factory** (UK): offizielle „Full Ingredient List"-PDF (Issue 20, 13/04/2026). KEIN Crawler. `py -3 bagelfactory-extract.py` (pdfplumber; Footer-Strip an „* Received frozen", Einträge über Seitengrenzen ok) → `data/bagelfactory-raw.json`; `node bagelfactory-update.js` → BAGELFACTORY-Block. À la carte (AC-Familie)
  - **NUR Set-Menü** (User-Entscheidung 04.07.2026): Die fertigen Menü-Bagels + Sweets haben **per-portion-Werte** (Fußnote der PDF: Werte gelten für den **Plain Bun**). Die EXTRAS- (BYO-Proteine/-Salate) und SAUCES-Sektionen sind **nur per-100g OHNE Portionsgewichte** → **BYO + Customizing der Menü-Bagels sind NICHT abbildbar** (keine Schätzwerte). Die ursprünglich geplanten BYO-Features (Bun-Mehrfachauswahl, „No sauce", „Standard-Salat Theo", Preisanzeige, Extras-Limits) entfallen deshalb — bei Bedarf reaktivierbar, falls Bagel Factory je Portionsgrößen publiziert
...
- **Bagel Factory**: `BAGELFACTORY.cats[]` + `BAGELFACTORY.items[]` (AC-Schema; 43 Items, 7 Kategorien, volle 8 Makros per Portion, Werte = Plain Bun). `optimizeBagelFactory(t,mode,p,activeCats,maxN,noSweets)` — Kategorie-Chips + Max-Bagels (1/2/3/4/5/∞) + Schalter „No snacks & sweet treats" (`noSweets` filtert cat `sweets`)
...
- **Bagel Factory**: alle 7 Kategorien aktiv, max. 5 Bagels (Chips 1/2/3/4/5/∞ — einziges Restaurant mit 4er-Stufe, User-Wunsch), „No snacks & sweet treats" AN; NUR Set-Menü (per-portion, Plain Bun), BYO nicht abbildbar; Drinks/Buns/Extras/Sauces nicht im Modell
