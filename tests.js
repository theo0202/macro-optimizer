...
check("Subway Cookie Oreo Cookie Cup (64g, 319 kcal)", (() => { const c = T.D.cookies.find(x => x.id === "cookie_oreo_cup"); return c && c.servingG === 64 && c.kcal === 319 && c.carbs === 38 && c.sat === 8.3; })(), true);
check("Subway Cookie Raspberry Cheesecake (208 kcal, fibre 0.2)", (() => { const c = T.D.cookies.find(x => x.id === "cookie_raspberry_cheesecake"); return c && c.kcal === 208 && c.fibre === 0.2 && c.protein === 2.5; })(), true);
check("Subway Cookie Rainbow Chocolate Chip (212 kcal)", T.D.cookies.find(c => c.id === "cookie_rainbow").kcal, 212);
check("Subway Cookie Double Chocolate (Ve) (217 kcal)", T.D.cookies.find(c => c.id === "cookie_double_choc_ve").kcal, 217);
check("Subway: Footlong Choc Chip Cookie entfernt", T.D.cookies.some(c => /footlong/i.test(c.name)), false);
...
check("Atis sauces (17: Lime Coriander Dressing raus)", T.ATIS.sauces.length, 17);
check("Atis saucesL (11 large Dressings, Lime Coriander raus)", T.ATIS.saucesL.length, 11);
check("Atis: kein Lime Coriander Dressing (User-Ausschluss)", T.ATIS.sauces.concat(T.ATIS.saucesL).every(x => !/coriander/i.test(x.name)), true);
...
// ── Wingstop (à la carte, AC-Familie; volle 8 Makros, '--' -> 0) ──
check("Wingstop Items (12)", T.WINGSTOP.items.length, 12);
check("Wingstop Kategorien (5)", T.WINGSTOP.cats.length, 5);
check("Wingstop volle 8 Makros (numerisch)", T.WINGSTOP.items.every(x => ["kcal", "fat", "sat", "carbs", "sugars", "fibre", "protein", "salt"].every(k => typeof x[k] === "number")), true);
check("Wingstop 10 Pieces Boneless (Tippfehler-Fix '0'->10, kcal 526)", (T.WINGSTOP.items.find(x => x.id === "10_pieces_boneless") || {}).kcal, 526);
check("Wingstop kein '0 Pieces Boneless' mehr", T.WINGSTOP.items.every(x => !/^0 pieces/i.test(x.name)), true);
check("Wingstop 8 Pieces Boneless protein (34)", (T.WINGSTOP.items.find(x => x.id === "8_pieces_boneless") || {}).protein, 34);
check("Wingstop Big Flavour Burger sugars (38)", (T.WINGSTOP.items.find(x => x.id === "big_flavour_burger") || {}).sugars, 38);
check("Wingstop Corn ohne Salt (salt=0)", T.WINGSTOP.items.filter(x => x.cat === "corn").every(x => x.salt === 0), true);
check("Wingstop Sweet Potato Fries ohne Protein (protein=0)", T.WINGSTOP.items.filter(x => /sweet potato/i.test(x.name)).every(x => x.protein === 0), true);
check("Wingstop sat <= fat ueberall", T.WINGSTOP.items.every(x => x.sat <= x.fat + 0.05), true);
check("Wingstop sugars <= carbs ueberall", T.WINGSTOP.items.every(x => x.sugars <= x.carbs + 0.05), true);
const wsBad = T.WINGSTOP.items.filter(x => { const e = 4 * x.carbs + 4 * x.protein + 9 * x.fat; return Math.abs(x.kcal - e) > 60 && Math.abs(x.kcal - e) / Math.max(x.kcal, 1) > 0.15; });
check("Wingstop kcal-plausibel (alle Items)", wsBad.length, 0);
const wsAll = {}; T.WINGSTOP.cats.forEach(c => wsAll[c.id] = true);
const tWs = { protein: 40, carbs: 45, fat: 18, kcal: 502, fibMin: null, fibMax: null, sMin: null, sMax: null };
const rWs = T.optimizeWingstop(tWs, "macros", {}, wsAll, 3);
check("Wingstop liefert Ergebnisse (1..20)", rWs.length > 0 && rWs.length <= 20, true);
...
// Optimizer: AC-Form + Kategorie-Filter
const puAll = {}; T.PURE.cats.forEach(c => puAll[c.id] = true);
const tPu = { protein: 40, carbs: 67, fat: 27, kcal: 671, fibMin: null, fibMax: null, sMin: null, sMax: null };
const rPu = T.optimizePure(tPu, "macros", {}, puAll, 3);
check("Pure Optimizer liefert Ergebnisse (AC-Form)", rPu.length > 0 && !!(rPu[0].items && rPu[0].nutrition && typeof rPu[0].score === "number"), true);
check("Pure Optimizer Top-1 = Chicken Shawarma Bowl (exakte Ziel-Makros)", rPu[0].items.length === 1 && rPu[0].items[0].id === "chicken_shawarma_pure_bowl", true);
check("Pure Kategorie-Filter (nur soups)", T.optimizePure(tPu, "macros", {}, { soups: true }, 2).every(r => r.items.every(x => x.cat === "soups")), true);
...
const RESTOS = ["subway", "farmerj", "itsu", "pret", "nandos", "ug", "wagamama", "gdk", "atis", "tfc", "chopstix", "pepes", "fiveguys", "pizzaexpress", "wasabi", "leon", "bagelfactory", "pho", "wingstop", "sushico", "pure"];
...
check("Search-Index deckt alle 21 Restaurants ab", new Set(T.SEARCH_INDEX.map(x => x.resto)).size, 21);
...
check("Waitrose Produkte (73)", T.WAITROSE.items.length, 73);
...
check("Waitrose BBQ Baked Beans (Merchant Gourmet, 200g, 94/100g -> 188 kcal)", (() => { const x = T.WAITROSE.items.find(y => y.id === "bbq_baked_beans"); return x && x.g === 200 && x.p100.kcal === 94 && x.p100.sugars === 2.6 && T.wtScale(x, 200).kcal === 188; })(), true);
check("Waitrose: BBQ + Classic Baked Beans sind getrennte Produkte", T.WAITROSE.items.filter(x => /baked beans/i.test(x.name)).length, 2);
// "London Nährwerte Waitrose Nummer 2.xlsx" — 10 Produkte (Werte live verifiziert; Marken #8/#9 korrigiert)
check("Waitrose Singapore Rice Noodles (275g, 133/100g)", (() => { const x = T.WAITROSE.items.find(y => y.id === "singapore_rice_noodles"); return x && x.brand === "Waitrose" && x.cat === "carbs_rice_grains" && x.g === 275 && x.p100.kcal === 133 && x.p100.salt === 0.21; })(), true);
check("Waitrose itsu Chicken Gyoza (itsu, gyoza, 150g)", (() => { const x = T.WAITROSE.items.find(y => y.id === "itsu_chicken_spring_onion_gyoza"); return x && x.brand === "itsu" && x.cat === "gyoza" && x.g === 150 && x.p100.protein === 8.2; })(), true);
check("Waitrose Veggie Bites Marke = All Plants (korrigiert)", (() => { const x = T.WAITROSE.items.find(y => y.id === "spinach_chickpea_veggie_bites"); return x && x.brand === "All Plants" && x.cat === "falafel" && x.p100.kcal === 209; })(), true);
check("Waitrose Feta Parcels Marke = Waitrose (korrigiert)", (() => { const x = T.WAITROSE.items.find(y => y.id === "spinach_feta_parcels"); return x && x.brand === "Waitrose" && x.cat === "gyoza" && x.g === 112 && x.p100.kcal === 270; })(), true);
check("Waitrose Edamame Bean Salad (vegetables, 190g, fibre 8.8)", (() => { const x = T.WAITROSE.items.find(y => y.id === "edamame_bean_salad"); return x && x.cat === "vegetables" && x.g === 190 && x.p100.fibre === 8.8; })(), true);
check("Waitrose Classic Baked Beans (Merchant Gourmet, fix 200g, 92/100g -> 184 kcal)", (() => { const x = T.WAITROSE.items.find(y => y.id === "classic_baked_beans"); return x && x.cat === "carbs_rice_grains" && x.brand === "Merchant Gourmet" && x.g === 200 && x.p100.kcal === 92 && T.wtScale(x, 200).kcal === 184 && T.wtScale(x, 200).protein === 10.2; })(), true);
...
check("Waitrose Chicken pieces = 8 Items", T.WAITROSE.items.filter(x => x.cat === "chicken_pieces").length, 8);
...
check("Waitrose Carbs/Rice/Grains = 33 Items (Sweetcorn Fritters raus)", T.WAITROSE.items.filter(x => x.cat === "carbs_rice_grains").length, 33);
check("Waitrose Cheese-Kategorie = 1 (nur Mozzarella; Ricotta Peppers raus)", T.WAITROSE.items.filter(x => x.cat === "cheese").length, 1);
...
check("Waitrose Falafel-Kategorie = 4 (Cauldron + Veggie Bites)", T.WAITROSE.items.filter(x => x.cat === "falafel").length, 4);
...
check("Waitrose Vegetables = 8 (+ Edamame/Mixed Bean Salad)", T.WAITROSE.items.filter(x => x.cat === "vegetables").length, 8);
check("Waitrose Gyoza-Kategorie = 5 (2 SD + 2 itsu + Feta Parcels)", T.WAITROSE.items.filter(x => x.cat === "gyoza").length, 5);
...
check("waitroseSearch leere Query -> Katalog + 3 Nuesse + Ghee (77)", T.waitroseSearch("", 90).length, 77);
...
check("waitroseOrderItems: 73 Items", wItems.length, 73);
...
check("WAITROSE_MENU hat 9 Kategorien + 73 Items", T.WAITROSE_MENU.cats.length === 9 && T.WAITROSE_MENU.items.length === 73, true);
...
check("Waitrose Veggie Roll cat sushi", T.WAITROSE.items.find(x => x.id === "veggie_roll").cat === "sushi", true);
check("Waitrose wtScale Veggie Roll@175g kcal (236.3)", T.wtScale(T.WAITROSE.items.find(x => x.id === "veggie_roll"), 175).kcal, 236.3);
check("Waitrose: Salmon Tartare/Veggie Roll kein Schalentier", !T.isShellfish(T.WAITROSE.items.find(x => x.id === "salmon_tartare_bowl")) && !T.isShellfish(T.WAITROSE.items.find(x => x.id === "veggie_roll")), true);
// Flamegrilled Chicken Pieces (fix 160g) -> kcal 138×1.6 = 220.8, protein 27.7×1.6 = 44.3
const wFlame = T.WAITROSE.items.find(x => x.id === "flamegrilled_chicken_pieces");
...
check("Waitrose wtScale Salmon Maki@108g kcal (184.7)", T.wtScale(T.WAITROSE.items.find(x => x.id === "salmon_maki"), 108).kcal, 184.7);
// Cucumber Maki @108g -> kcal 113×1.08 = 122; salt 0.04×1.08 = 0
check("Waitrose wtScale Cucumber Maki@108g kcal (122)", T.wtScale(T.WAITROSE.items.find(x => x.id === "cucumber_maki"), 108).kcal, 122);
// Cooked Green Thai Chicken Pieces: fixes 160g-Produkt -> kcal 132×1.6 = 211.2, protein 25.2×1.6 = 40.3
const wThai = T.WAITROSE.items.find(x => x.id === "green_thai_chicken_pieces");
check("Waitrose Green Thai Chicken fix 160g", wThai.variable === false && wThai.g === 160, true);
...
check("WAITROSE_MEALS: 43 Mahlzeiten", T.WAITROSE_MEALS.length, 43);
...
check("waitroseMealResults: 43 Ergebnisse in AC-Form", mealRes.length === 43 && mealRes.every(r => r.items && r.nutrition && typeof r.score === "number"), true);
// Batch 9: Gyoza/Fried-Rice-Kombis (m45-m50)
check("m45 (itsu Gyoza+Edamame+SweetChilli+StickyRice) 4 Items ~849.3 kcal", (() => { const r = mealRes.find(x => x._meal === "m45"); return r && r.items.length === 4 && Math.abs(r.nutrition.kcal - 849.3) < 0.6; })(), true);
check("m50 (EggFriedRice+EdamameSalat+Flamegrilled) ~807.7 kcal", (() => { const r = mealRes.find(x => x._meal === "m50"); return r && Math.abs(r.nutrition.kcal - 807.7) < 0.6; })(), true);
check("Batch 9: alle verbliebenen Produkt-ids existieren", ["m45","m46","m48","m49","m50"].every(id => T.WAITROSE_MEALS.find(m => m.id === id).items.every(it => T.WAITROSE.items.some(w => w.id === it.id))), true);
...
check("Batch 8: alle verbliebenen Produkt-ids existieren", ["m35","m37","m38","m40","m41","m44"].every(id => T.WAITROSE_MEALS.find(m => m.id === id).items.every(it => T.WAITROSE.items.some(w => w.id === it.id))), true);
...
check("filterMeals mustSD: 11 Mahlzeiten mit >=1 Sushi Daily", T.filterMeals(mealRes, "all", "", false, true, false).length, 11);
...
// Batch 4: neue Küche "Middle Eastern" + 9 neue Mahlzeiten (inkl. 4-Produkt-Kombis + Sushi-Daily)
check("WAITROSE_CUISINES enthält Middle Eastern", T.WAITROSE_CUISINES.includes("Middle Eastern"), true);
check("Mahlzeiten mit cuisine Middle Eastern = 2", mealRes.filter(r => r._cuisine === "Middle Eastern").length, 2);
const m16r = mealRes.find(r => r._meal === "m16"), m21r = mealRes.find(r => r._meal === "m21"), m24r = mealRes.find(r => r._meal === "m24");
check("m16 (Katsu+EggNoodles+Greens) 3 Items ~800.8 kcal", m16r.items.length === 3 && Math.abs(m16r.nutrition.kcal - 800.8) < 0.6, true);
check("m21 (Gyoza+StickyRice+Edamame+GreenThai) 4 Items ~755 kcal", m21r.items.length === 4 && Math.abs(m21r.nutrition.kcal - 755.4) < 0.6, true);
...
check("filterMeals noSD: 11 Mahlzeiten mit Sushi Daily raus -> 32", T.filterMeals(mealRes, "all", "", true).length, 32);
...
check("filterMeals noSD=false: alle 43 (Gegenprobe)", T.filterMeals(mealRes, "all", "", false).length, 43);
...
check("filterMeals 'all' -> alle 43", T.filterMeals(mealRes, "all", "").length, 43);
