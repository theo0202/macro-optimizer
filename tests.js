...
check("Waitrose Produkte (13)", T.WAITROSE.items.length, 13);
...
// Suche
check("waitroseSearch 'sushi daily' findet die 11 Sushi-Daily-Produkte", T.waitroseSearch("sushi daily", 60).length, 11);
...
check("waitroseSearch leere Query -> ganzer Katalog", T.waitroseSearch("", 60).length, 13);
...
check("waitroseOrderItems: 13 Items", wItems.length, 13);
...
check("WAITROSE_MENU hat 1 Kategorie + 13 Items", T.WAITROSE_MENU.cats.length === 1 && T.WAITROSE_MENU.items.length === 13, true);
